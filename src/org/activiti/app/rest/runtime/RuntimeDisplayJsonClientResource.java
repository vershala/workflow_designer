...
public class RuntimeDisplayJsonClientResource {
  ...
  public JsonNode getModelJSON(@PathVariable String processInstanceId) {
    ...
    List<String> completedFlows = gatherCompletedFlows(activityInstances, pojoModel);
  ...
  protected List<String> gatherCompletedFlows(List<HistoricActivityInstance> activityInstances, BpmnModel pojoModel) {

    List<String> completedFlows = new ArrayList<String>();
    
    // key为从起点ID-终点ID，value为线的ID
 	Map<String, String> allLineMap = new HashMap<>();
 	for(org.activiti.bpmn.model.Process process : pojoModel.getProcesses()){
 		 for(FlowElement fe : process.getFlowElements()){
 			if (fe instanceof SequenceFlow) {
 		        SequenceFlow flow = (SequenceFlow) fe;
 		        allLineMap.put(flow.getSourceRef()+"-"+flow.getTargetRef(), flow.getId());
 		    }
 		 }
 	}
    
    int len = activityInstances.size(),
		firstLen = len - 1 ;

	for (int i = 0; i < firstLen; i++) {
		// 得 到节点定义的详细信息
		HistoricActivityInstance activityIns = activityInstances.get(i);
		for (int j = i + 1; j < len; j++) {
			HistoricActivityInstance flowActivityIns = activityInstances.get(j);
			if (allLineMap.get(activityIns.getActivityId()+"-"+flowActivityIns.getActivityId())!=null) {
				completedFlows.add(allLineMap.get(activityIns.getActivityId()+"-"+flowActivityIns.getActivityId()));
			}
			if(flowActivityIns.getTaskId() != null){
				break;
			}
		}
