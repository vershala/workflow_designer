    ...
    .controller('FormBuilderController', ['$rootScope', '$scope', '$translate', '$http', '$timeout', '$location', '$modal', '$routeParams', '$popover', 'FormBuilderService',
        function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal, $routeParams, $popover, FormBuilderService) {
            ...
            function setFieldDragDropAttributes (field, prefix) {
                ...
                // update by Asy - set default field id
                if(!field.id) {
                	field.id = field._guid;
                }
            ...
            $scope.onFieldDrop = function (paletteElementOrField, dropArrayTarget, event, index) {
                ...
                // update by Asy - upload、group or people only can be loaded once
                var label = "Label",id;
                if($scope.formElements != null && (fieldId == "upload" || fieldId == "people" || fieldId == "functional-group")){
                	var tp;
                	for(var i=0 ;i < $scope.formElements.size();i++){
                		tp = $scope.formElements[i].type;
                		if(tp == fieldId || fieldId == "people" && tp == "functional-group" || fieldId == "functional-group" && tp == "people"){
                			$rootScope.addAlert("附件上传、处理人(或处理组)只能加载一次","warn");
                			return false;
                		}
                	}
                	if(fieldId == "upload"){
                		...
                		label = "下一处理人员";
                		id = "nextHandler_";
                	}else if(fieldId == "functional-group"){
                		label = "下一处理组";
                		id = "nextHandleGroup_";
                	}
                }
                
                if (fieldId === 'radio-buttons' || fieldId === 'dropdown' || fieldId === 'boolean') {
                ...
                setFieldDragDropAttributes(field, 'field');
            ...
            $scope.palletteElements = [
                ...
                {'type': 'amount', 'title': $translate.instant('FORM-BUILDER.PALLETTE.AMOUNT'), 'icon': 'images/form-builder/amountfield-icon.png', 'width': 1},
                ...
                {'type': 'functional-group', 'title': $translate.instant('FORM-BUILDER.PALLETTE.GROUP-OF-PEOPLE'), 'icon': 'images/form-builder/peoplegroup-icon.png', 'width': 1},
