        ...
        link: function ($scope, $element, attributes) {
            ...
            $scope.formTabs = [
                ...
                },
                {
                    ...
                    "show": ['dropdown', 'radio-buttons', 'boolean']
                },
                {
                    "id": "upload",
            ...
            $scope.openFieldPopover = function () {
                ...
                //update by Asy - set id and name is not same
                $scope.formElement.overrideId = true;
            ...
            $scope.doneEditing = function () {
            	...
            	// update by Asy - validate the same id
            	var items = $rootScope.formItems;
                var ele = $scope.formElement;
                for(var it = 0;it < items.size();it++){
                	if(items[it]._guid != ele._guid && items[it].id != null && items[it].id == ele.id){
                		$scope.errorMessage = "字段ID已存在，请重新设置";
                		return false;
                	}
                }
                ...
                if ($scope.fieldEditPopup) {
                    ...
                    // update by Asy - handle more than one modal showing at the same time
                    var eles = jQuery.find(".modal.ng-scope.top.am-fade");
                    if(eles.length > 0){
                    	for(var i=0 ;i<eles.length; i++){
                    		jQuery(eles[i]).remove();
                    	}
                    }
                    var mds = jQuery.find(".modal-backdrop.am-fade");
                    if(mds.length > 0){
